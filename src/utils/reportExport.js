...
const COLUMNS = [
  { key: 'no',       header: '#',         pdfWidth: 26,  xlsxWidth: 5,  align: 'center' },
  { key: 'fullName', header: 'Full Name', pdfWidth: 148, xlsxWidth: 26, align: 'center' },
  { key: 'gender',   header: 'Gender',    pdfWidth: 56,  xlsxWidth: 10, align: 'center' },
  { key: 'center',   header: 'Center',    pdfWidth: 84,  xlsxWidth: 16, align: 'center' },
  { key: 'site',     header: 'Site',      pdfWidth: 78,  xlsxWidth: 15, align: 'center' },
  { key: 'gradYear', header: 'Grad Year', pdfWidth: 60,  xlsxWidth: 11, align: 'center' },
  { key: 'mode',     header: 'Mode',      pdfWidth: 66,  xlsxWidth: 12, align: 'center' },
  { key: 'phone',    header: 'Phone',     pdfWidth: 104, xlsxWidth: 18, align: 'center' },
  { key: 'city',     header: 'City',      pdfWidth: 68,  xlsxWidth: 14, align: 'center' },
];
...
export function exportDisciplesToPdf({ disciples, filterSummary, generatedBy }) {
  ...
  // ── Letterhead band ──
  // Everything in the band and the meta strip below it reads as a single
  // centered unit -- logo + title + subtitle as one block, then the
  // filter summary and generated-by line each centered on their own
  // line -- to match the centered banner used in the Excel export.
  const bandHeight = 82;
  ...
  doc.setFontSize(19);
  const titleText = 'BCC International Database';
  const titleWidth = doc.getTextWidth(titleText);
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(11.5);
  const subtitleText = 'Disciples Report';
  const subtitleWidth = doc.getTextWidth(subtitleText);
  const textBlockWidth = Math.max(titleWidth, subtitleWidth);

  // The unit is [logo][gap][text block]; center that whole unit on the page.
  const unitWidth = logoRadius * 2 + logoGap + textBlockWidth;
  const unitStartX = pageCenterX - unitWidth / 2;
  const logoCenterX = unitStartX + logoRadius;
  const textStartX = unitStartX + logoRadius * 2 + logoGap;

  // Circular "BCC" logo mark, drawn rather than embedded so the file
  // never depends on an external image loading correctly.
  doc.setFillColor(255, 255, 255);
  doc.circle(logoCenterX, bandHeight / 2, logoRadius, 'F');
  doc.setTextColor(...BRAND.rgb);
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(11.5);
  doc.text('BCC', logoCenterX, bandHeight / 2 + 4, { align: 'center' });

  doc.setTextColor(255, 255, 255);
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(19);
  doc.text(titleText, textStartX, bandHeight / 2 - 6);
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(11.5);
  doc.text(subtitleText, textStartX, bandHeight / 2 + 15);
...
export function exportDisciplesToExcel({ disciples, filterSummary, generatedBy }) {
  ...
  const META_ROW = 2;
  const HEADER_ROW = 4; // one blank row above it
  ...
  worksheet['!rows'] = [{ hpt: 26 }, { hpt: 20 }, { hpt: 18 }, { hpt: 6 }, { hpt: 22 }];
